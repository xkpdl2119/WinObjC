...
string _ArrayToNSArrayConvertor::instantiator(const std::shared_ptr<ClangObjectModel::TypeInfo> typeInfo) {
    ...
    string nullCheck = "    if (val == nullptr) {\n"
        "        return nil;\n"
        "    }\n";
    return nullCheck + "    std::unique_ptr<" + ci->adapterObjBase + "> adapter = std::make_unique<" + ci->adapterObj + "<" + GetWRLTypeNameForContainerTemplates(typeInfo) + ",\n    " +
        ci->implClassName + "<" +
        GetWRLTypeNameForContainerTemplates(tp, true, false) +
        ", " +
...
string _MapViewToNSDictionaryConvertor::instantiator(const std::shared_ptr<ClangObjectModel::TypeInfo> typeInfo) {
    ...
    string keyEnumeratorAdapterTypeT = string("DictionaryKeyEnumeratorAdapterObj<RTMapViewKeyEnumerator<") +
        wrlK + "," +
        wrlK + "," +
        objcK + ", " +
        wrlV + ">>";

    string baseTypeT = ci->implClassName + "<" +
        wrlK + "," +
        wrlK + ", " +
        wrlV + ", " +
        wrlV + ", " +
        ...
        objcV + ", " +
        GetWRLTypeNameForContainerTemplates(typeInfo) + ", " +
        keyObjCConvertorFunc + ", " +
        valObjCConvertorFunc + ", " +
        keyWRLConvertorFunc + ", " +
        valWRLConvertorFunc + ">";
...
string WinrtType(std::shared_ptr<ClangObjectModel::TypeInfo> typeInfo, bool baseType, bool forIDL) {
    ...
    if (typeInfo->getSymbolKind() == ClangObjectModel::JSONType ||
        typeInfo->getTypeSpelling(true) == "NSError") {
...
string WinRTContainerType(std::shared_ptr<ClangObjectModel::TypeInfo> typeInfo, bool baseType, bool forIDL) {
    ...
    if (typeInfo->templateParams.size() == 0)
    {
        ...
        errorMsg << "Containers must be strongly typed for a WinRT component.  Please add annotations to the method or generics to the API definition. ";
...
std::string getWRLConvertorFuncForType(const std::shared_ptr<ClangObjectModel::TypeInfo> typeInfo, const std::string& callString) {
    ...
    if (typeInfo->isHeterogeneous) {
        auto it = g_globalData.currentComponent->getHeterogeneousContainerInfo(typeInfo);
        convertor = ClangHelpers::generateResolvedTypeName(it->second->toWRLConvertor, g_globalData.currentComponent->getSDKName(), false, "");
        return "static_cast<" + it->second->wrlContainerName + "<" + it->second->wrlTemplateParams + ">*>(" + convertor + "(" + callString + ").Detach())";
    } else if (typeInfo->isContainer()) {
        ...
        if(iface->second->DerivesFromUIElement()) {
            ...
            std::string runtimeClass = ClangHelpers::generateResolvedTypeName(g_winrtClassPrefix + typeInfo->getTypeSpelling(true), typeInfo->getSDKName(), false, "");
            std::string ABIInterface = ClangHelpers::generateResolvedTypeName("I" + typeInfo->getTypeSpelling(true), typeInfo->getSDKName(), false);
            ...
            ss << "return " << temp << " != nil ? Make<" << resolvedType << ">(_" << typeInfo->getTypeName() << ").Detach() : nullptr;" << endl;
            ss << "}()";
            return ss.str();
        }
        ...
        return "static_cast<IMapView<HSTRING, IInspectable*>*>(" + g_commonConvertors + "::convertNSDictionaryToPropertySet(" +
            callString + ").Detach())";
    } else if (typeInfo->isNSURLType()) {
        if (typeInfo->getTypeSpellingLowerCase(true) == "uri") {
            return g_commonConvertors + "::convertNSURLToWinRTUri(" + callString +
                ").Detach()";
        } else {
        ...
        if (iface == nullptr) {
            ...
            std::string resolvedType = ClangHelpers::generateResolvedTypeName(g_winrtClassPrefix +
                Helpers::Trim(Helpers::baseType(iface->name, true)),
                iface->getSDKName(),
                false,
                "");
...
string GetWRLTypeCreatorName(std::shared_ptr<ClangObjectModel::TypeInfo> typeInfo) {
    ...
    if (containerInfo) {
        ...
        return ClangHelpers::generateNamespaceString(ifaceInfo->second->getSDKName(), "::") +
            "::" + "_Get" + g_winrtClassPrefix + Helpers::Trim(Helpers::baseType(typeInfo->getTypeSpelling())) + "ForObjC";
...
string GetObjCTypeCreatorName(std::shared_ptr<ClangObjectModel::TypeInfo> typeInfo) {
    if (typeInfo->isHeterogeneous) {
        ...
        return ClangHelpers::generateNamespaceString(ifaceInfo->second->getSDKName(), "::") + 
            "::" + "_GetObjCFor" + g_winrtClassPrefix + Helpers::Trim(Helpers::baseType(typeInfo->getTypeSpelling()));
    }
