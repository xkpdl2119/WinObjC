...
void updateAsyncClassInfo(std::shared_ptr<ClangObjectModel::ParameterInfo>& param, std::shared_ptr<ClangObjectModel::MethodInfo>& methodInfo) {
    //Try to determine the return type
    param->marshallDelegateAsAsync = true;
    methodInfo->UpdateMethodNameForIDLAndRuntimeClass(methodInfo->getMethodNameForIdl() + "Async", methodInfo->getMethodNameForRuntimeClass() + "Async");
...
bool isInterface(const std::string& typeSpelling) {
    ...
    if (ClangObjectModel::GlobalData::synthesizedInterfaceMap.find(baseType) != ClangObjectModel::GlobalData::synthesizedInterfaceMap.end()) {
        return true;
    }
...
std::string generateCallToIslandWoodInit() {
    ...
    string appDelegateName = g_globalData.applicationDelegateInfo != nullptr ? "L\"_" + g_globalData.applicationDelegateInfo->ifaceInfo->name + "\"" : "nullptr";
