...
map<string, string> NSNumberToPropertyValue = {
    ...
    {"BOOL", "CreateBoolean"},
    { "char", "CreateUInt8"},
    { "wchar_t", "CreateUInt16" },
    { "unsigned char", "CreateUInt8"},
    { "short", "CreateInt16"},
    { "unsigned short", "CreateUInt16"},
    { "int", "CreateInt32"},
    { "unsigned int", "CreateUInt32"},
    { "float", "CreateSingle" },
    { "double", "CreateDouble" },
    { "NSInteger", "CreateInt32" },
    { "NSUInteger", "CreateUInt32" },
    { "long", "CreateInt32" },
    { "unsigned long", "CreateUInt32" },
    { "long long", "CreateInt64" },
    { "INT16", "CreateInt16" },
    { "INT32", "CreateInt32" },
    { "INT64", "CreateInt64" },
    { "UINT16", "CreateUInt16" },
...
std::string generateObjCWrapperDeclaration(std::shared_ptr<ClangObjectModel::ComponentInfo> comp) {
    ...
    for (auto containerMap : comp->heterogeneousContainers) {
        ...
        // The ObjC wrapper.
        ss << "@interface " << objCInterfacePrefix << info->name << " : " << info->objCContainerName <<
            " {" << endl;
        ss << "    ComPtr<IInspectable> comObj;" << endl;
        ss << "}" << endl;
...
std::string generateRTWrapperDeclaration(std::shared_ptr<ClangObjectModel::ComponentInfo> comp) {
    ...
    for (auto containerMap : comp->heterogeneousContainers) {
        ...
        ssInterfaceDecl << "class " + rtClassName + " : public RuntimeClass<" << info->wrlContainerName << "<" << info->wrlTemplateParams << ">>, public GetInnerObject {" << endl;
...
std::string _generateRTWrapperMethodDeclarationForNSArray(std::shared_ptr<ClangObjectModel::HeterogeneousContainerInfo> info) {
    ...
    ss << "    virtual HRESULT STDMETHODCALLTYPE GetMany(unsigned int startIndex, unsigned int capacity, IInspectable** values, "
        "unsigned int* actual) override;" << endl;
...
std::string _generateHeterogeneousCreatorForObjC(std::shared_ptr<ClangObjectModel::HeterogeneousContainerInfo> info) {
    ...
    std::string resolvedType = ClangHelpers::generateResolvedTypeName(rtClassPrefix + info->name,
...
string _generateObjCWrapperMethodDefinitionsForNSDictionaryKeyEnumerator(shared_ptr<ClangObjectModel::HeterogeneousContainerInfo> info) {
    ...
    objCMarshallingHelperName = ClangHelpers::generateResolvedTypeName(objCMarshallingHelperName, g_globalData.currentComponent->getSDKName(), false, "");
...
string _generateObjCMarshallingForPropertyValueTypes(std::set<shared_ptr<ClangObjectModel::TypeInfo>, ClangObjectModel::TypeInfo> types, string inspectableObject, string objectName) {
    ...
    for (auto typeInfo : types) {
        ...
        if (typeInfo->getTypeSpelling() == "NSArray") {
            ...
            ssBody << "                id _returnObj = " << TypeConvertor::GetObjCTypeCreatorName(typeInfo) << "(" << objectName << ".Get());" << endl;
...
std::string _generateObjCMarshallingForInterfaceTypes(std::shared_ptr<ClangObjectModel::TypeInfo> typeInfo, string inspectableObject, std::string objectName) {
    stringstream ss;
    auto ifaceInfo = g_globalData.interfaceMap.find(Helpers::Trim(Helpers::baseType(typeInfo->getTypeSpelling())));
    std::string resolvedType = ClangHelpers::generateResolvedTypeName("I" +
        TypeConvertor::WinrtType(typeInfo, true),
...
std::string _generateObjCMarshallingForContainerTypes(std::shared_ptr<ClangObjectModel::TypeInfo> typeInfo, string inspectableObject, std::string objectName) {
    stringstream ss;

    ss << "    ComPtr<" << Helpers::baseType(TypeConvertor::WinrtType(typeInfo, true), true) << "> " << objectName << ";" << endl;
    ss << "    if (SUCCEEDED(" << inspectableObject << ".As(&" << objectName << "))) {" << endl;
    std::string convertorName = ClangHelpers::generateResolvedTypeName(TypeConvertor::GetObjCTypeCreatorName(typeInfo), g_globalData.currentComponent->getSDKName(), false, "");
...
std::string _generateObjCWrapperMethodDefinitionsForNSDictionary(std::shared_ptr<ClangObjectModel::HeterogeneousContainerInfo> info) {
    ...
    ss << "    THROW_NS_IF_FAILED(ABI::Windows::Foundation::GetActivationFactory(HString::MakeReference"
        "(L\"Windows.Foundation.PropertyValue\").Get(), &inst));" << endl;
    ...
    rtMarshallingHelperName = ClangHelpers::generateResolvedTypeName(rtMarshallingHelperName, g_globalData.currentComponent->getSDKName(), false, "");
...
string _generateRTMarshallingForInterfaceTypes(shared_ptr<ClangObjectModel::TypeInfo> typeInfo,
    string objectName) {
    stringstream ss;
    ...
    ss << "    if ([" << objectName << " isKindOfClass:[" << Helpers::Trim(Helpers::baseType(typeInfo->getTypeSpelling())) << " class]]) {" << endl;
    ss << "        return " << TypeConvertor::GetWRLTypeCreatorName(typeInfo) <<
        "(" << objectName << ");" << endl;
    ss << "    }" << endl;
    ss << endl;
    return ss.str();
...
string _generateRTMarshallingForContainerTypes(shared_ptr<ClangObjectModel::TypeInfo> typeInfo,
    string objectName) {
    stringstream ss;
...
std::string _generateRTWrapperMethodDefinitionsForNSArray(std::shared_ptr<ClangObjectModel::HeterogeneousContainerInfo> info) {
    ...
    // GetMany
    ss << "STDMETHODIMP " << rtClassPrefix << info->name <<
        "::GetMany(unsigned int startIndex,\n"
        "    unsigned int capacity,\n"
        "    IInspectable** values,\n"
        "    unsigned int* actual) {" << endl;
    ss << generateABIBoundaryStart();
