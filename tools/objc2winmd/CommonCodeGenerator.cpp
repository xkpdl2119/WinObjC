...
string generateNSNotificationCenterIDL() {
    ...
    for (auto handler : g_globalData.notifications) {
        ...
        ssEvents << "    [eventadd] HRESULT " << handler << "([in] " << delegateName << "* handler, [out] [retval] EventRegistrationToken* token);" << endl;
...
string generateNSNotificationCenterDeclarations() {
    ...
    for (auto notification : g_globalData.notifications) {
        ...
        ss << endl;
        ss << "class " << g_winrtClassPrefix << notification << " : public RuntimeClass<" << rootNamespace <<
            "::I" << notification << ">, public GetInnerObject {" << endl;
...
string generateNSNotificationCenterDefinitions() {
    ...
    for (auto notification : g_globalData.notifications) {
        ...
        ss << "    " << resolvedDelegateName << "* handler," << endl;
        ss << "    " << "EventRegistrationToken* token) {" << endl;
        ...
        ss << "}" << endl;
        ss << endl;
        ss << "class " << rtStaticClassName << " : public ActivationFactory<ABI::" << rootNamespace << "::" << staticInterfaceName << "> {" << endl;
        ...
        ss << "        if (SUCCEEDED(propertySetMap->Lookup(nsStrToHstr(" << notification << ").Get(), &inspectablePropertyValue))) {" << endl;
        ...
        ss << "            RETURN_IF_FAILED(propertySetMap->Insert(nsStrToHstr(" << notification << ").Get(), inspectablePropertyValue.Get(), &replaced));" << endl;
        ss << "        }" << endl;
        ss << "        return S_OK;" << endl;
        ss << generateABIBoundaryEnd();
        ss << "    }" << endl;
