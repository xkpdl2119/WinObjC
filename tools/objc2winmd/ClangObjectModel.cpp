...
std::unordered_map<std::string, std::shared_ptr<ClangObjectModel::InterfaceInfo>> GlobalData::objCSuperClasses = {
    { "UIButton", std::make_shared<ClangObjectModel::InterfaceInfo>("UIButton", "ABI::Windows::UI::Xaml::Controls::ContentControl", "Windows.UI.Xaml.Controls.ContentControl", "windows.ui.xaml.controls.idl") }
};
...
std::unordered_map<std::string, std::shared_ptr<ClangObjectModel::InterfaceInfo>> GlobalData::_objCFrameworkInterfaces = {
    { "nsdate", std::make_shared<ClangObjectModel::InterfaceInfo>("NSDate", "ABI::Windows::Foundation::DateTime", "Windows.Foundation.DateTime", "windows.foundation.idl") },

    // Our AST parser will convert NSURL to either StorageFile or URI or both.
    { "storagefile", std::make_shared<ClangObjectModel::InterfaceInfo>("NSURL", "ABI::Windows::Storage::IStorageFile*", "Windows.Storage.StorageFile*", "windows.storage.idl")},
    { "uri", std::make_shared<ClangObjectModel::InterfaceInfo>("NSURL", "ABI::Windows::Foundation::IUriRuntimeClass*", "Windows.Foundation.Uri*", "windows.foundation.idl")}
};

// maps the container name to number of template types for it.
// e.g. array has a single tempate type.
//      dictionary is a key-value pair.
static std::map<std::string, int> supportedHeterogeneousTypes = {
    {"NSArray", 1},
    {"NSDictionary", 2} };
...
string MetaDataComments::_parseTemplateArgumentTypes(const std::string& line, const std::string typeString, const std::string& objCContainerName) {
    ...
    // char
    vector<string> possibleKeyTypes = Helpers::getStringBetweenSymbols(templateTypes[0], possibleTypesStartSymbol, possibleTypesEndSymbol, true);
    ...
    string name = g_globalData.currentComponent->getSDKName() +
        g_globalData.currentComponent->SDKHeaderFile +
        "HeterogeneousContainer" + objCContainerName +
        to_string(g_globalData.currentComponent->heterogeneousContainers.size());
...
void MetaDataComments::_processRawComments(const std::string& rawComments) {
    ...
    for (auto line : lines) {
        ...
        } else if (metaDataFor == MetaDataType_ApplicationDelegate ||
            metaDataFor == MetaDataType_DelegateAllocator ||
            metaDataFor == MetaDataType_DelegateInitializer ||
            metaDataFor == MetaDataType_DelegateAllocatorAndInitializer) {
...
map<string, shared_ptr<HeterogeneousContainerInfo>>::iterator
    ComponentInfo::getHeterogeneousContainerInfo(std::shared_ptr<ClangObjectModel::TypeInfo> typeInfo) {
...
void addObjcCreatorFuncsFromType(std::shared_ptr<ClangObjectModel::TypeInfo> typeInfo, std::unordered_map<std::string, std::shared_ptr<ClangObjectModel::TypeInfo>>& objCCreatorFuncs) {
...
void addObjcCreatorFuncsFromParameter(std::shared_ptr<ClangObjectModel::ParameterInfo> paramInfo, std::unordered_map<std::string, std::shared_ptr<ClangObjectModel::TypeInfo>>& objCCreatorFuncs) {
...
void addObjcCreatorFuncsFromMethods(std::vector<std::shared_ptr<ClangObjectModel::MethodInfo>>& methods, std::unordered_map<std::string, std::shared_ptr<ClangObjectModel::TypeInfo>>& objCCreatorFuncs) {
...
{
    AddOrUpdateReference<ClangObjectModel::InterfaceInfo>(interfaceName, true, shared_from_this(), g_globalData.interfaceMap, g_globalData.unresolvedInterfaces);
}

void ComponentInfo::AddReferenceToProtocol(const std::string & protocolName)
{
    AddOrUpdateReference<ClangObjectModel::ProtocolInfo>(protocolName, false, shared_from_this(), g_globalData.protocolMap, g_globalData.unresolvedProtocols);
}
...
void TypeInfo::updateObjCCreatorFunctionName(std::string methodName,
    ...
    if (!methodName.empty()) {
        objCCreatorFunc.insert({ methodName,
            typeInfo });
    }
}
...
bool TypeInfo::hasBaseWinRTRepresentation() {
    if (isContainer() ||
        getTypeSpelling(true) == "NSError" ||
        getSymbolKind() == JSONType) {
        return true;
    }
...
bool TypeInfo::isContainer() {
    if (TypeConvertor::GetContainerInfo(_typeSpelling) != nullptr ||
        isHeterogeneous) {
        return true;
    }
...
bool AsyncClassInfo::operator() (const std::shared_ptr<ClangObjectModel::AsyncClassInfo>& info1, const std::shared_ptr<ClangObjectModel::AsyncClassInfo>& info2) const {
    return (info1->name < info2->name);
}

std::shared_ptr<ClangObjectModel::AsyncClassInfo> 
AsyncClassInfo::CloneAsyncClassInfo(const std::shared_ptr<ClangObjectModel::AsyncClassInfo>& info) {
...
std::shared_ptr<ClangObjectModel::ParameterInfo> ParameterInfo::CloneParameterInfo(const std::shared_ptr<ClangObjectModel::ParameterInfo>& info) {
...
PropertyInfo::PropertyInfo(const std::string& propertyName, const std::string& getterName, const std::string& setterName, std::shared_ptr<ClangObjectModel::MetaDataComments> metaData)
{
