...
std::string generateDelegateParamList(const std::vector<std::shared_ptr<ClangObjectModel::TypeInfo>>& delegateParams, bool includeIdlDecorations = true) {
    std::string paramList;

    if (delegateParams.size() > 0) {
        paramList = Helpers::addSeparators(delegateParams, [&](const std::shared_ptr<ClangObjectModel::TypeInfo> t) {
            std::string typeName;
            if (includeIdlDecorations) {
                ...
                if (t->isHeterogeneous) {
                    auto it = g_globalData.currentComponent->heterogeneousContainers.find(t->getTypeSpelling(true));
...
std::string marshalToObjC(const std::shared_ptr<ClangObjectModel::ParameterInfo> paramInfo) {
    if (paramInfo->typeInfo->typeKind == CXType_BlockPointer) {
        ...
        if (paramInfo->marshallDelegateAsAsync) {
            if (paramInfo->asyncInfo->returnTypes.size() < 2) {
                ...
                string resolvedImplName = ClangHelpers::generateResolvedTypeName(paramInfo->asyncInfo->implName,
                    paramInfo->asyncInfo->sdkName,
                    false,
                    "");
                string localVarName = "_" + paramInfo->asyncInfo->implName;
                return blockType + " { spAsyncWorker->setResult(^" + resolvedImplName + "*() {ComPtr<" + resolvedImplName + "> " + localVarName +
                    " = Make<" + resolvedImplName + ">(); (" + localVarName + ".Get())->Init(" + delegateParamNameListForWinRT + "); return " +
                    localVarName + ".Detach();}());}";
            }
        } else {
            ...
            return blockType + " { " +
                "ComPtr<" + comPtrType + "> " + comPtrName + "; " +
                comPtrName + ".Attach(" + paramInfo->typeInfo->getTypeName() + "); " +
                "THROW_NS_IF_FAILED(" + comPtrName + "->Invoke(" + delegateParamNameListForWinRT + ")); }";
        }
    }
    ...
    if (paramInfo->typeInfo->isNSURLType()) {
        return g_commonConvertors + "::convertWinRT" + paramInfo->typeInfo->getTypeSpelling(true) +
            "ToNSURL(" + paramInfo->typeInfo->getTypeName() + ")";
    }
    ...
    if (ClangHelpers::isInterface(paramInfo->typeInfo->getTypeSpelling())) {
        ...
        std::string resolvedType = ClangHelpers::generateResolvedTypeName(g_winrtClassPrefix + TypeConvertor::WinrtType(paramInfo->typeInfo, true),
...
string getParamList(std::vector<shared_ptr<ClangObjectModel::ParameterInfo>>& params, bool generateForIDL) {
    return Helpers::addSeparators(params, [&](const std::shared_ptr<ClangObjectModel::ParameterInfo> p) {
            ...
            if (p->marshallDelegateAsAsync) {
                if (generateForIDL) {
                    ...
                    if (!resolvedImplNameForIDL.empty()) {
                        asyncReturnParam = "[out] [retval] Windows.Foundation.IAsyncOperation<" + resolvedImplNameForIDL + ">** __returnValue";
                    }
                    else {
    ...
    if (generateReturnType) {
        ...
        if (method->returnType->typeInfo->typeKind != CXType_Void) {
            ...
            if (ClangHelpers::isInterface(method->returnType->typeInfo->getTypeSpelling()) ||
                ...
                returnParam = ClangHelpers::generateResolvedTypeName(prefix + returnParam, method->returnType->typeInfo->getSDKName(), generateForIDL);
            }
...
std::string generateInterfaceIdl(const std::string& IDLInterfaceName,
    ...
    for (const auto& method : methods) {
        ...
        ss << "    " << method->idlAnnotation << "HRESULT " << methodName << "(" << generateParamList(method, true, true, isInitMethod) << ");" << std::endl;
    }
...
std::string generateInterfaceForAsyncClassIdl(const std::string& IDLInterfaceName,
    ...
    for (const auto& method : methods) {
        ...
        ss << "    " << method->idlAnnotation << "HRESULT " << methodName << "(" << generateParamList(method, true, true, false) << ");" << std::endl;
    }
...
std::string generateAppDelegateCreator() {
    ...
    if (g_globalData.applicationDelegateInfo->allocatorMethod == nullptr &&
        ...
        if (g_globalData.applicationDelegateInfo->allocatorMethod != g_globalData.applicationDelegateInfo->initializerMethod) {
            ...
            appDelegateCreator = "[[" + g_globalData.applicationDelegateInfo->ifaceInfo->name + " " + g_globalData.applicationDelegateInfo->allocatorMethod->name + "] " +
        ...
        appDelegateCreator = "[[" + g_globalData.applicationDelegateInfo->ifaceInfo->name + " " + g_globalData.applicationDelegateInfo->allocatorMethod->name + "] init]";
    } else {
        // use [[interface alloc] initializerMethod]
        appDelegateCreator = "[[" + g_globalData.applicationDelegateInfo->ifaceInfo->name + " alloc] " + g_globalData.applicationDelegateInfo->initializerMethod->name + "]";
    }
...
std::string generateAppDelegateImpl() {
    stringstream ss;
    ss << "#import <" << g_globalData.applicationDelegateInfo->ifaceInfo->getSDKName() << "/" << g_globalData.applicationDelegateInfo->ifaceInfo->SDKHeaderFile << ".h>" << endl;
    ss << "#import \"" << g_sdkParameters.rootNameSpace << "_" << g_globalData.applicationDelegateInfo->ifaceInfo->name << ".h\"" << endl << endl;
...
std::string generateFactoryMethodBody(const std::shared_ptr<ClangObjectModel::MethodInfo> methodInfo, const std::shared_ptr<ClangObjectModel::InterfaceInfo> iface, const std::string paramName) {
    stringstream ss;

    ss << "        *" << paramName << " = Make<" << ClangHelpers::generateNamespaceString(iface->getSDKName(), "::") << "::" << g_winrtClassPrefix << iface->name << ">(";
...
std::string generateClassFactory(const std::shared_ptr<ClangObjectModel::InterfaceInfo> iface) {
    ...
    if (iface->hasDefaultInitializer) {
        // default constructor:
        std::string runtimeClass = ClangHelpers::generateResolvedTypeName(g_winrtClassPrefix + Helpers::baseType(iface->name, true), iface->getSDKName(), false, "");
        std::string ABIInterface = ClangHelpers::generateResolvedTypeName("I" + Helpers::baseType(iface->name, true), iface->getSDKName(), false);
    ...
    for (const auto& method : iface->parameterizedInitMethods) {
        ss << "    STDMETHODIMP " << generateFactoryMethodName(method->objectiveCSelector) << "(" << generateParamList(method, false, false) <<
            ", " << ClangHelpers::generateResolvedTypeName("I" + iface->name, iface->getSDKName()) << "** " << paramName << ") override {" << std::endl;
        ss << generateABIBoundaryStart();
        ss << ClangHelpers::generateCallToIslandWoodInit();
    ...
    for (const auto& method : iface->classMethods) {
        ss << "    STDMETHODIMP " << method->getMethodNameForRuntimeClass() << "(" << generateParamList(method, false) << ") override {" << std::endl;
        ...
        if (usesDelegateProtocols) {
            if (!method->returnType->typeInfo->isInterface()) {
                ...
                localVarsCode << "        auto " << tempVar << " = " << TypeConvertor::getWRLConvertorFuncForType(method->returnType->typeInfo, callStr) + ";" << endl;
                if (!localVarsToSave.empty()) {
                    for (const auto& typeInfo : localVarsToSave) {
                        saveVarsCode << tempVar + "->" + typeInfo->generateMemberVarName() + " = " + typeInfo->generateObjcLocalVarName() << ";" << endl;
                    }
                }
...
string generateAsyncClassInitMethod(const shared_ptr<ClangObjectModel::AsyncClassInfo>& asyncInfo) {
    ...
    for (auto item : asyncInfo->returnTypes) {
        if (item->isComType()) {
            ss << "        " << "_" << ClangHelpers::getSynthesizedAsyncClassPropertyName(item) << ".Attach(" << item->getTypeName() << ");" << endl;
        } else {
            ss << "        " << "_" << ClangHelpers::getSynthesizedAsyncClassPropertyName(item) << " = " << item->getTypeName() << ";" << endl;
        }
    }
    ss << "    }" << endl;
    ss << endl;
...
string generateAsyncClassInstanceMethod(shared_ptr<ClangObjectModel::MethodInfo> method) {
    ...
    if (method->returnType->typeInfo->isComType()) {
        ...
        ss << "        *" << method->returnType->typeInfo->getTypeName() << " = _" << ClangHelpers::getSynthesizedAsyncClassPropertyName(method->returnType->typeInfo) << ";" << endl;
    }
    ss << "        return S_OK;" << endl;
    ss << "    }" << endl;
    ss << endl;
...
std::string generateInstanceMethod(const std::string classNamePrefix, std::shared_ptr<ClangObjectModel::MethodInfo> method) {
    ...
    ss << "STDMETHODIMP " << classNamePrefix << method->getMethodNameForRuntimeClass() << "(" << generateParamList(method, false) << ") {" << std::endl;
    ss << generateABIBoundaryStart();
...
std::string generateClass(const std::shared_ptr<ClangObjectModel::InterfaceInfo> iface) {
    ...
    if (iface->parameterizedInitMethods.size() > 0) {
        for (const auto& method : iface->parameterizedInitMethods) {
            ss << classNamePrefix << g_winrtClassPrefix << iface->name << "(" << generateParamList(method, false, false) << ") {" << std::endl;
    ...
    if (iface->DerivesFromUIElement()) {
        ...
        ss << "    RETURN_IF_FAILED(ABI::Windows::Foundation::GetActivationFactory(HStringReference(RuntimeClass_Windows_UI_Xaml_Controls_ContentControl).Get(), &baseFactory));" << std::endl;
...
std::string generateObjCCreatorFunctions(const std::unordered_map<std::string, std::shared_ptr<ClangObjectModel::TypeInfo>>& creatorFuncMap) {
    stringstream ss;
    ...
    for (auto ifaceInfo : g_globalData.currentComponent->ifaces) {
        ...
        std::string runtimeClass = ClangHelpers::generateResolvedTypeName(g_winrtClassPrefix + Helpers::baseType(ifaceInfo.first, true), ifaceInfo.second->getSDKName(), false, "");
        std::string ABIInterface = ClangHelpers::generateResolvedTypeName("I" + Helpers::baseType(ifaceInfo.first, true), ifaceInfo.second->getSDKName(), false);
        ...
        //All Winrt interfaces derive from IInspectable.  When a runtime class implements/subclasses 2 or more interfaces, we have a diamond problem as both interfaces
...
std::string generateHeaders(const std::shared_ptr<ClangObjectModel::InterfaceInfo>& iface) {
    ...
    auto classNameStr = "RuntimeClass_" + ClangHelpers::generateNamespaceString(g_globalData.currentComponent->getSDKName(), "_") + "_" + iface->name;
    ...
    while (interfaceIter != nullptr && (g_globalData.objCSuperClasses.find(interfaceIter->name) == g_globalData.objCSuperClasses.end())) {
        ...
        implementedInterfaces += (implementedInterfaces.empty() ? "" : ", ") + ClangHelpers::generateResolvedTypeName("I" + interfaceIter->name, interfaceIter->getSDKName());
    ...
    // The class:
    ss << "class " << g_winrtClassPrefix << iface->name << " : public RuntimeClass<"
        << implementedInterfaces
        << (iface->DerivesFromUIElement() ? ", ComposableBase<>" : "")
        << ">, public GetInnerObject {" << std::endl;
    ...
    for (auto delMap : g_globalData.currentComponent->delegateCallbackProtocolsMap) {
        ...
        for (auto instMethod : delMap.second->instanceMethods) {
            ...
            for(unsigned int i=0; i < instMethod->params.size(); i++) {
                ...
                ss << "    auto " << temp << " = " << TypeConvertor::getWRLConvertorFuncForType(param->typeInfo, param->typeInfo->getTypeName()) << ";" << endl;
...
std::string generateClassBindings(const std::shared_ptr<ClangObjectModel::ComponentInfo> component) {
    ...
    ss << "#include <ComIncludes.h>" << std::endl;
    ss << "#include \"" << ClangHelpers::generateNamespaceString(component->getSDKName(), ".")
        << "." << component->SDKHeaderFile << ".h\"" << std::endl;
    ...
    for (auto iface : component->ifaces) {
        std::string headerName = ClangHelpers::generateNamespaceString(component->getSDKName(), ".") +
            "." + component->SDKHeaderFile + "." + iface.first + ".h";
    ...
    for (auto protocolName : component->referencedProtocols) {
        ...
        if (iface == nullptr) {
            ...
            if (it == g_globalData.delegateCallbackProtocolsMap.end() ||
                it->second->implHeaderName.empty()) {
            ...
            headerName = ClangHelpers::generateNamespaceString(iface->getSDKName(), ".") +
                "." + iface->SDKHeaderFile + "." + iface->name + ".h";
        }
        headerNames.insert(headerName);
    }
    ...
    for (const auto& asyncClass : g_globalData.currentComponent->asyncClasses) {
        ...
        if (asyncClass->synthesizedInterface != nullptr) {
            ...
            ss << "    InspectableClass(L\"Windows.Foundation.IAsyncOperation<" << resolvedInterfaceName << "*>\", BaseTrust); " << std::endl;
...
int applyMetaDataInfo(std::shared_ptr<ClangObjectModel::MethodInfo>& methodRef,
    ...
    if (count > 1) {
        ...
        bool isMetaDataForReturnType = (metaDataInfo->getMetaDataType(currentMetaDataLine) == ClangObjectModel::MetaDataType_ReturnType) ||
    ...
    for (int j = 0; j < count; j++) {
        ...
        currentNumber = applyMetaDataInfo(methodRef,
            methodInfo,
            metaDataInfo,
            info,
            isFromApplicationDelegate,
            currentMetaDataLine + 1,
            currentNumber);
    }
...
void handleEnums(const CXCursor cursor, std::shared_ptr<ClangObjectModel::ComponentInfo>& component) {
    ...
    if (it != g_globalData.enums.end()) {
        ...
        if (enumInfo->enumConstants.size() &&
            (enumInfo->getSDKName() != component->getSDKName() ||
            ...
            component->referencedIDLs.insert(ClangHelpers::generateNamespaceString(enumInfo->getSDKName(), ".") + "." + enumInfo->SDKHeaderFile + ".idl");
            return;
        }
    } else {
...
void _parsePropertyDeclaration(const string& src, const string& propertyName, string& getterName, string& setterName) {
    ...
    std::regex regGetter("(?:[\\(])(?:[a-zA-Z0-9_, =:]*)(?:getter{1})(?:[[:s:]]*)(?:={1})(?:[[:s:]]*)([a-zA-Z0-9_]+)(?:[a-zA-Z0-9_, =:]*)(?:[\\)])");
    ...
    std::regex regSetter("(?:[\\(])(?:[a-zA-Z0-9_, =:]*)(?:setter{1})(?:[[:s:]]*)(?:={1})(?:[[:s:]]*)([a-zA-Z0-9_]+)(?:[a-zA-Z0-9_, =:]*)(?:[\\)])");
...
std::shared_ptr<ClangObjectModel::PropertyInfo> HandlePropertyDecl(CXCursor cursor) {
    ...
    std::shared_ptr<ClangObjectModel::PropertyInfo> propInfo = make_shared<ClangObjectModel::PropertyInfo>(propertyName, getterName, setterName, metadata);
    return propInfo;
}
...
std::shared_ptr<ClangObjectModel::MethodInfo> HandleMethodDecl(CXCursor cursor,
    ...
    if (method->returnType->typeInfo->getTypeSpelling(true) == "NSURL" &&
        metaDataInfo->getReturnMetaData().empty()) {
        // There is no metadata for the return type.
        // Inject our custom metadata.
        metaDataInfo->appendMetaData(ClangObjectModel::MetaDataType_ReturnType, -1, -1, 
            vector<string>({ "StorageFile", "Uri"}));
    }
    ...
    for (size_t i = 0; i < method->params.size(); i++) {
        ...
        if (param->typeInfo->getTypeSpelling(true) == "NSURL" &&
            !metaDataInfo->parameterHasMetaData(i)) {
            // There is no metadata on this parameter.
            // Inject our custom meta data.
            metaDataInfo->appendMetaData(ClangObjectModel::MetaDataType_Parameter, i, -1, 
                vector<string>({ "StorageFile", "Uri"}));
        }
    }
...
CXChildVisitResult ProtocolVisitor(CXCursor cursor, CXCursor parent, CXClientData client_data) {
    ...
    case CXCursor_ObjCInstanceMethodDecl: {
        ...
        std::shared_ptr<ClangObjectModel::MetaDataComments> metaDataInfo;
        std::shared_ptr<ClangObjectModel::MethodInfo> method = HandleMethodDecl(cursor, protoInfo->name, protoInfo->properties, metaDataInfo);
        ...
        applyMetaDataInfo(method, protoInfo->instanceMethods, metaDataInfo, make_shared<ClangObjectModel::MetaDataComments>(*metaDataInfo), false);
    }
    break;
    }
...
CXChildVisitResult InterfaceVisitor(CXCursor cursor, CXCursor parent, CXClientData client_data) {
    ...
    case CXCursor_ObjCClassMethodDecl: {
        ...
        std::shared_ptr<ClangObjectModel::MetaDataComments> metaDataInfo;
        std::shared_ptr<ClangObjectModel::MethodInfo> method = HandleMethodDecl(cursor, ifaceInfo->name, ifaceInfo->properties, metaDataInfo);
        ...
        if (isInitMethod) {
            ...
            applyMetaDataInfo(method, ifaceInfo->parameterizedInitMethods, metaDataInfo, make_shared<ClangObjectModel::MetaDataComments>(*metaDataInfo), ifaceInfo->isApplicationDelegate);
        } else {
            if (kind == CXCursor_ObjCInstanceMethodDecl) {
                applyMetaDataInfo(method, ifaceInfo->instanceMethods, metaDataInfo, make_shared<ClangObjectModel::MetaDataComments>(*metaDataInfo), ifaceInfo->isApplicationDelegate);
            } else {
                applyMetaDataInfo(method, ifaceInfo->classMethods, metaDataInfo, make_shared<ClangObjectModel::MetaDataComments>(*metaDataInfo), ifaceInfo->isApplicationDelegate);
            }
        }
        break;
    }
...
CXChildVisitResult InterfaceCategoryVisitor(CXCursor cursor, CXCursor parent, CXClientData client_data) {
    ...
    case CXCursor_ObjCClassRef:
        ...
        if (it == g_globalData.interfaceMap.end()) {
            ...
            stringstream errMsg;
            errMsg << "Could not find interface declaration for category " << name << ". Please inlcude the header for the category." << std::endl;
...
void HandleTypedefDecl(CXCursor cursor) {
    ...
    do {
        ...
        if (it == g_globalData.currentComponent->typedefDecl.end()) {
            stringstream errMsg;
            errMsg << "Typedef " << underlyingTypeSpelling << " used before it is defined. Please inlcude the header for the typedef." << std::endl;
            Helpers::Errors::WriteError(cursor, errMsg.str(), false);
        }
...
void HandleReferences(CXCursor cursor, std::shared_ptr<ClangObjectModel::ComponentInfo>& component) {
    ...
    case CXCursor_EnumDecl: {
            ...
            if (sdkName == component->getSDKName() &&
                fname == component->SDKHeaderFile) {
...
// Libclang has some weird (buggy?) behaviour with @class <interfaceName>.  The cursor for a forward decl is a ObjcInterfaceDecl instead of a ObjCClassRef
// In order to differentiate @class <interfacename> from a class with only a defualt initializer (e.g. @interface Foo{}), we need to visit the cursor's children,
...
void FixUnresolvedReferences(const std::string& referenceName, bool isInterface, std::unordered_multimap<std::string, std::shared_ptr<ClangObjectModel::ComponentInfo>> unresolvedMap) {
    ...
    while (componentIt != it.second) {
        ...
        if (isInterface) {
            ...
            componentIt->second->referencedInterfaces.insert(namespaceString + "." + g_globalData.currentComponent->SDKHeaderFile + "." + referenceName);
        }
        else {
            // generate the interface header name and put it in referencedProtocols.
            componentIt->second->referencedProtocols.insert(namespaceString + "." + g_globalData.currentComponent->SDKHeaderFile + "." + referenceName);
        }
    ...
    case CXCursor_ObjCProtocolDecl: {
        ...
        if(!isForwardDecl) {
            ...
            if (metaData.isDelegateCallbackProtocol()) {
                protocol->implHeaderName = ClangHelpers::generateNamespaceString(protocol->getSDKName(), ".") + "." + protocol->SDKHeaderFile + "." +
                    protocol->name + ".h";
...
void runTests(std::string outputDir) {
    ...
    if (g_globalData.applicationDelegateInfo) {
        dumpFile(g_sdkParameters.rootNameSpace + "_" + g_globalData.applicationDelegateInfo->ifaceInfo->name + ".mm", generateAppDelegateImpl(), outputDir);
        dumpFile(g_sdkParameters.rootNameSpace + "_" + g_globalData.applicationDelegateInfo->ifaceInfo->name + ".h", generateAppDelegateDecl(), outputDir);
    }
    ...
    if (g_globalData.requiresNSNotificationMarshalling) {
        ...
        dumpFile(rootNamespace + "._" + g_notificationCenter + ".mm", CommonCodeGenerator::generateNSNotificationCenterDefinitions(), outputDir);
    }
    ...
    for (auto delPair : g_globalData.delegateCallbackProtocolsMap) {
        string fileName = ClangHelpers::generateNamespaceString(delPair.second->getSDKName(), ".") + "." +
            delPair.second->SDKHeaderFile + "." + delPair.second->name + ".h";
    ...
    for (auto comp : g_globalData.components) {
        ...
        if (comp->ifaces.size() > 0 || comp->protocols.size() > 0 || comp->enums.size() > 0) {
            ...
            if (comp->heterogeneousContainers.size()) {
                dumpFile(namespaceString + "." + comp->SDKHeaderFile + "._Containers.h", generateHeterogeneousContainerDeclarations(comp), outputDir);
            }
        }
        comp->clearAll();
    }
...
int main(int argc, char** argv) 
{
    ...
    for (int i = 1;i < argc;i++)
    {
        ...
        if (indexPeriod == -1 || 
            (indexSlash != -1 && indexPeriod <= indexSlash))
        {
            Usage();
        }
